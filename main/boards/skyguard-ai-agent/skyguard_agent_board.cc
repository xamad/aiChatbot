...
#define TAG "SkyGuardAgent"

/*
 * SkyGuard AI Agent — Agentic Astronomy Copilot
 * ================================================
 * Based on LCDWiki 2.8" ESP32-S3 Display Board
 * Server: skyguardai.xamad.net (multi-agent swarm)
 *
...
class SkyGuardAgentBoard : public WifiBoard {
private:
    Button boot_button_;
    ...
    static std::string ProvideWebUIStatus(void* ctx) {
        auto* board = (SkyGuardAgentBoard*)ctx;
    ...
    void InitializeNetworkServices() {
        ...
        // SQM server URL for posting readings (public SQM data)
        ...
        // Agent server URL for telemetry (multi-agent swarm)
        agent_server_url_ = sg_settings.GetString("agent_server_url", "https://skyguardai.xamad.net");
        if (!agent_server_url_.empty()) {
            ESP_LOGI(TAG, "Agent server URL: %s", agent_server_url_.c_str());
        }
    ...
    std::string agent_server_url_;   // skyguardai.xamad.net telemetry endpoint
    ...
    void ReloadConfigFromNvs() {
        ...
        // Agent server URL
        agent_server_url_ = sg.GetString("agent_server_url", "https://skyguardai.xamad.net");
    ...
    void PostReadingToServer() {
        ...
        cJSON_AddStringToObject(dev, "type", "agent");
        // MAC address
        uint8_t mac[6];
        ...
        if (posted) {
            // Sent OK — try flushing any cached readings
            if (cache_count_ > 0) {
                ESP_LOGI(TAG, "Server reachable — flushing %d cached readings", cache_count_);
                FlushReadingCache();
            }
        } else {
            // Failed — cache the reading for later (takes ownership of a copy)
            char* cached_copy = cJSON_strdup(json);
            if (cached_copy) CacheReading(cached_copy);
        }

        // POST telemetry to agent server (parallel, fire-and-forget)
        if (!agent_server_url_.empty()) {
            char agent_url[128];
            snprintf(agent_url, sizeof(agent_url), "%s/api/device/readings", agent_server_url_.c_str());
            char* agent_resp = SkyGuardHttp::AllocBuffer(256);
            if (agent_resp) {
                bool agent_ok = SkyGuardHttp::Post(agent_url, json, agent_resp, 256, 10000);
                if (agent_ok) {
                    ESP_LOGI(TAG, "Agent telemetry posted OK");
                } else {
                    ESP_LOGW(TAG, "Agent telemetry POST failed (non-critical)");
                }
                free(agent_resp);
            }
        }
    ...
    void InitializeSkyGuardDisplay() {
        ...
        // Floating star tap → start listening (invoke Sophia without wake word)
        sg_display_->SetFloatingStarCallback([](void* ctx) {
            auto& app = Application::GetInstance();
            auto state = app.GetDeviceState();
            if (state != kDeviceStateListening &&
                state != kDeviceStateSpeaking &&
                ...
                ESP_LOGI(TAG, "Floating star tapped → StartListening");
                app.StartListening();
            }
        }, this);
        ...
        esp_timer_create_args_t timer_args = {
            .callback = [](void* arg) {
                auto* board = (SkyGuardAgentBoard*)arg;
                    ...
                    if (lvgl_port_lock(50)) {
                        ...
                        if (scr) {
                            ...
                            lv_obj_add_event_cb(scr, [](lv_event_t* e) {
                                ...
                                auto* b = (SkyGuardAgentBoard*)lv_event_get_user_data(e);
                ...
                board->sg_display_->SetFloatingStarVisible(!ai_active);
                ...
                // GPS map tile fetch — every 30s for first 5 min, then every 10 min
                {
                    ...
                    if (!ai_active && gps_try && board->sg_display_) {
                        ...
                        if (board->sg_display_->NeedsGpsMapImage(lat, lon)) {
                            ...
                            struct GpsMapCtx { SkyGuardAgentBoard* b; float lat; float lon; };
                        ...
                        if (!board->geocode_done_ && !board->google_api_key_.empty()) {
                            ...
                            if (board->sg_display_->HasPosition(rlat, rlon)) {
                                ...
                                struct GeoCtx { SkyGuardAgentBoard* b; float lat; float lon; };
...
public:
    SkyGuardAgentBoard() : boot_button_(BOOT_BUTTON_GPIO) {
        ESP_LOGI(TAG, "=============================================");
    ...
    virtual std::string GetBoardJson() override {
        auto& wifi = WifiManager::GetInstance();
        // Send "skyguard-ai-agent" as type to webhook
        std::string json = R"({"type":"skyguard-ai-agent",)";
...
DECLARE_BOARD(SkyGuardAgentBoard);
