...
NoAudioCodecDuplex::NoAudioCodecDuplex(int input_sample_rate, int output_sample_rate, gpio_num_t bclk, gpio_num_t ws, gpio_num_t dout, gpio_num_t din, gpio_num_t mclk) {
    duplex_ = true;
    input_sample_rate_ = input_sample_rate;
    output_sample_rate_ = output_sample_rate;

    ESP_LOGI(TAG, "Creating duplex audio codec with MCLK on GPIO %d", mclk);
    ESP_LOGI(TAG, "BCLK=%d, WS=%d, DOUT=%d, DIN=%d", bclk, ws, dout, din);
    ...
    ESP_ERROR_CHECK(i2s_channel_init_std_mode(tx_handle_, &std_cfg));
    ESP_ERROR_CHECK(i2s_channel_init_std_mode(rx_handle_, &std_cfg));
    ESP_LOGI(TAG, "Duplex channels created with MCLK");
}
...
int NoAudioCodec::Write(const int16_t* data, int samples) {
    ...
    if (data_bit_width_ == I2S_DATA_BIT_WIDTH_16BIT) {
        ...
        // Volume scaling: linear 0-100 -> 0.0-2.0 (with 2x gain boost for MAX98357A)
        float volume_scale = float(output_volume_) / 100.0f * 2.0f;
